...
const MOCK_ACCEPTED_INCIDENTS: AcceptedIncident[] = [
  {
    id: "4",
    title: "Building Fire",
    description: "Small fire reported in residential building at Barangay Catiningan",
    location: {
  ...
  },
  {
    ...
    description: "Elderly person needs immediate medical assistance in Barangay Pasi",
    location: {
  ...
  },
  {
    id: "8",
    title: "Power Outage",
    description: "Electrical post damaged, causing power outage in several barangays",
    location: {
      latitude: 13.0584 - 0.008,
...
const MOCK_RESOLVED_INCIDENTS: ResolvedIncident[] = [
  ...
  },
  {
    id: "7",
    title: "Flooding",
    description: "Rising water levels affecting residential area after heavy rainfall",
    location: {
...
const ReportsScreen = () => {
  ...
  const handleIncidentPress = (incident: Incident): void => {
    console.log(`ReportsScreen: Navigating to incident details - ID: ${incident.id}, Title: ${incident.title}`)
  ...
  const renderIncidentCard = ({ item }: { item: AcceptedIncident | ResolvedIncident }): JSX.Element => {
    console.log(`ReportsScreen: Rendering incident card - ID: ${item.id}, Title: ${item.title}`)
      ...
      >
        <View style={styles.cardHeader}>
          <View style={styles.cardTitleContainer}>
            {item.type && <View style={[styles.incidentTypeIndicator, { backgroundColor: item.type.color }]} />}
        ...
        <View style={styles.cardContent}>
          <View style={styles.imageContainer}>
            ...
            {item.type && (
              <View style={[styles.incidentTypeIcon, { backgroundColor: item.type.color }]}>
                <MaterialCommunityIcons name={item.type.icon} size={16} color="#FFFFFF" />
              </View>
            )}
          </View>
          ...
          <View style={styles.cardDetails}>
            ...
            <View style={styles.cardMetadata}>
              <View style={styles.metadataItem}>
                <MaterialCommunityIcons name="map-marker" size={14} color="#2C74B3" />
              ...
              <View style={styles.metadataItem}>
                <MaterialCommunityIcons name="clock-outline" size={14} color="#2C74B3" />
                <Text style={styles.metadataText}>{getTimeElapsed(item.timestamp)}</Text>
              </View>
  ...
  const getCurrentData = () => {
    ...
    return activeTab === TABS.ACCEPTED ? MOCK_ACCEPTED_INCIDENTS : MOCK_RESOLVED_INCIDENTS
  }
      ...
      <View style={styles.container}>
        <View style={styles.header}>
          <View style={styles.headerTitleContainer}>
            <MaterialCommunityIcons name="clipboard-list" size={22} color="#2C74B3" style={styles.headerIcon} />
        ...
        <View style={styles.tabContainer}>
          <TouchableOpacity style={styles.tab} onPress={() => handleTabChange(TABS.ACCEPTED)} testID="tab-active">
          ...
          <TouchableOpacity style={styles.tab} onPress={() => handleTabChange(TABS.RESOLVED)} testID="tab-resolved">
            <View style={styles.tabContent}>
              <MaterialCommunityIcons
          ...
          <Animated.View
            style={[
              styles.tabIndicator,
              {
                ...
                backgroundColor: activeTab === TABS.ACCEPTED ? "#2C74B3" : "#4CAF50",
              },
            ]}
          />
        </View>
        ...
        <View style={styles.statsContainer}>
          <View style={styles.statItem}>
            <Text style={styles.statValue}>
              {activeTab === TABS.ACCEPTED ? MOCK_ACCEPTED_INCIDENTS.length : MOCK_RESOLVED_INCIDENTS.length}
            </Text>
            <Text style={styles.statLabel}>{activeTab === TABS.ACCEPTED ? "Active Reports" : "Resolved Reports"}</Text>
          </View>
          ...
          <View style={styles.statItem}>
            <Text style={styles.statValue}>
              {activeTab === TABS.ACCEPTED ? Math.floor(Math.random() * 60) + 30 : Math.floor(Math.random() * 120) + 60}
              m
            </Text>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            refreshControl={
              <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={["#2C74B3"]} tintColor="#2C74B3" />
            }
            ListEmptyComponent={
